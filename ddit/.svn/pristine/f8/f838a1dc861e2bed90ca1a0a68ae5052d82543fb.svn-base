...
public class InterviewController {
	...
	public ResponseEntity<List<MemberVO>> proAdminListByTime(@RequestParam("start_date")String start_date,@RequestParam("end_date")String end_date ){
		...
		try {
			proList = memberService.getProAdminByTime(param);
			entity = new ResponseEntity<List<MemberVO>>(proList, HttpStatus.OK);
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public void regist(InterviewRegistRequest interviewReq, HttpServletResponse response, HttpSession session,
		...
		System.out.println(interviewReq.getInterview_start_date()+"-------------------------------------");
		...
		for (int i = 0; i < cnt; i++) {
			...
			interview.setInterview_name(classes.getCurriculum_name() + classes.getClass_number() + "기  면접");
			...
			System.out.println("시작시간 :"+ interview.getInterview_start_date());
			interview.setInterview_end_date(
					new Date(startdate.getTime() + (long) (1000 * 60 * interviewReq.getInterview_time() * (i + 7))));
			System.out.println("시작시간 :"+ interview.getInterview_end_date());
