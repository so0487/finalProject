...
public class ZoomApiIntegration {
	...
	String REDIRECT_URI_CREATE_INTERVIEW = ZoomDetails.getRedirectUriCreate_interview();
	String REDIRECT_URI_CREATE_LECTURE = ZoomDetails.getRedirectUriCreate_lecutre();
	String REDIRECT_URI_CREATE_COUNSEL = ZoomDetails.getRedirectUriCreate_counsel();
	...
	public String getAuthorizationUrl(HttpSession session) {
		...
		if (zoom.equals("zoom")) {
			url = REDIRECT_URI;
		} else if (zoom.equals("zoom_create_interview")) {
			url = REDIRECT_URI_CREATE_INTERVIEW;
		} else if (zoom.equals("zoom_create_lecture")) {
			url = REDIRECT_URI_CREATE_LECTURE;
		} else if (zoom.equals("zoom_create_counsel")) {
			url = REDIRECT_URI_CREATE_COUNSEL;
		...
		OAuth20Service oauthService = new ServiceBuilder().apiKey(CLIENT_ID).apiSecret(CLIENT_SECRET).callback(url)
	...
	public OAuth2AccessToken getAccessToken(HttpSession session, String code, String state) throws IOException {
		...
		if (StringUtils.pathEquals(sessionState, state)) {
			...
			OAuth20Service oauthService = new ServiceBuilder().apiKey(CLIENT_ID).apiSecret(CLIENT_SECRET).callback(url)
					.state(state).build(ZoomDetails.instance());
