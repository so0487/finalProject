...
getPage("<%=request.getContextPath()%>/replies/board/declare/${post.post_no}/"+replyPage);
...
$('#replyAddBtn').on('click',function(e){
	   ...
	   $.ajax({
	      ...
	      success:function(data){
	         ...
	         if(result[0]=="SUCCESS"){
	            alert('댓글이 등록되었습니다.');
	            getPage("<%=request.getContextPath()%>/replies/board/declare/${post.post_no}/"+result[1]);
...
$('#replyModBtn').on('click',function(event){
   ...
   $.ajax({
      url:"<%=request.getContextPath()%>/replies/board/declare/"+reply_no,
      type:"put",
