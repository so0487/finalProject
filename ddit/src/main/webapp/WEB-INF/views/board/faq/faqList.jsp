		...
		<div class="row justify-content-between align-items-center mb-10">
			...
			<div style="z-index: 1; position: absolute; top: 92px; right: 30px;">
				<c:if test="${loginUser.member_role eq 'ADMIN'}">
					<button class="btn btn-outline" style="border:1px solid #dddddd;" onclick="location.href='<%=request.getContextPath()%>/board/faq/registForm'">
						<span>등록</span>
					</button>
				</c:if>
			</div>
					...
					<div class="tab-content">
						<div id="keyword" class="card-tools" style="width:600px; margin-left:15px; margin-bottom:10px;">
							<div class="input-group row">
								...
								<select class="form-control col-md-4" name="searchType" id="searchType">
									<option value="" ${pageMaker.cri.searchType eq 'tcw' ? 'selected':'' }>전체</option>
						...
						<div class="tab-pane fade show active" id="course">
							<table class="table">
								<thead>
									<tr>
										<th data-name="post_no">번 호</th>
										<th data-name="post_title">제 목</th>
								...
								<c:forEach items="${postList}" var="post">
									<tbody>
										<tr>
											...
											<td style="width:60%"><a href="javascript:location.href='<%=request.getContextPath() %>/board/faq/detail.page?post_no=${post.post_no}'">${post.post_title}</a></td>
