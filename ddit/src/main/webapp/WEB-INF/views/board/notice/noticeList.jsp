		...
		<div class="row justify-content-between align-items-center mb-10">
			...
			<div style="z-index: 1; position: absolute; top: 92px; right: 30px;">
				<c:if test="${loginUser.member_role eq 'ADMIN'}">
					<button class="btn btn-outline" style="border:1px solid #dddddd;" onclick="location.href='<%=request.getContextPath()%>/board/notice/registForm'">
						<span>등록</span>
					</button>
				</c:if>
			</div>
								...
								<c:forEach items="${postList}" var="post">
									<tbody>
										<tr>
											...
											<td style="width:50%"><a href="javascript:location.href='<%=request.getContextPath() %>/board/notice/detail.page?post_no=${post.post_no}'">${post.post_title}</a></td>
