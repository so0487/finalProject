       ...
       <!-- Page Headings Start -->
        <div class="row justify-content-between align-items-center mb-10">
       	...
       	<div style="z-index:1; position: absolute; top: 85px; right: 30px;">
			<button id="regBtn"  style="border:1px solid #dddddd" class="btn btn-outline"  onclick="location.href='my/list.do'">
				<span>내 상담 보기</span>
			</button>
		</div>
	                         ...
	                         <table class="table">
	                                   ...
	                                   <tr>
	                                       ...
	                                       <th data-breakpoints="xs" data-name="curriculum_time">상담제목</th>
	                                        ...
	                                        <c:if test="${member.member_role eq 'STUDENT'}">
	                                        	<c:if test="${member.member_role eq 'STUDENT'}">
	                                       			<th data-breakpoints="xs" data-name="" >신청</th>
	                                       		</c:if>
	                                       </c:if>
	                                   </tr>
	                               </thead>
	                               <tbody>
										<c:forEach items="${counselList }" var="counsel" varStatus="status">
											<c:if test="${counsel.counsel_status eq '0' && counsel.counsel_student eq null }">
			                                   <tr>
...
<%-- 			                                        	<c:if test="${counsel.counsel_student eq null }"> --%>
				                                       		<td><a href=""><button type="button" class="badge badge-info" id="applyBtn" name ="applyBtn">신청</button></a></td>
<%-- 				                                    	</c:if> --%>
<%-- 			                                        	<c:if test="${counsel.counsel_student ne null }"> --%>
<!-- 				                                       		<td><a href=""><button type="button" class="button button-info" disabled="disabled" style="background-color: gray;" >신청마감</button></a></td> -->
<%-- 				                                    	</c:if> --%>
				                                    </c:if>
			                                   </tr>
			                                </c:if>
